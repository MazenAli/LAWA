...
int main () {
    ...
    cout << "Wavelets: d = " << d << ", d_ = " << d_ << ", dmw = " << dmw << ", j0 = " << j0 << endl << endl; 

    //getchar();

    /// Basis initialization
    Basis_Y     basis_y(d,d_,j0);
    Basis_X     basis_x(dmw,0);
    ...
    Function<T> chi_omega_0_Fct(chi_omega_0, singPts);
    Function<T> chi_omega_1_Fct(chi_omega_1, singPts);
    ...
    /// Right Hand Side:
    DenseVectorT sing_support1(1);
    DenseVectorT sing_support2(2);
    DenseVectorT sing_support3(1);

    DenseVectorT sing_support4(1);
    DenseVectorT sing_support5(2);
    DenseVectorT sing_support6(1);
    ...
    ///      Forcing Functions
    SeparableFunction2D<T> F1_Fct(f_1_x, sing_support1, f_1_y, sing_support4);
    SeparableFunction2D<T> F2_Fct(f_1_x, sing_support1, f_2_y, sing_support5);
    SeparableFunction2D<T> F3_Fct(f_1_x, sing_support1, f_3_y, sing_support6);
    SeparableFunction2D<T> F4_Fct(f_2_x, sing_support2, f_1_y, sing_support4);
    SeparableFunction2D<T> F5_Fct(f_2_x, sing_support2, f_2_y, sing_support5);
    SeparableFunction2D<T> F6_Fct(f_2_x, sing_support2, f_3_y, sing_support6);
    SeparableFunction2D<T> F7_Fct(f_3_x, sing_support3, f_1_y, sing_support4);
    SeparableFunction2D<T> F8_Fct(f_3_x, sing_support3, f_2_y, sing_support5);
    SeparableFunction2D<T> F9_Fct(f_3_x, sing_support3, f_3_y, sing_support6);
    ...
    lhs_theta_fcts.push_back(theta_2);
    ThetaStructure<ParamType> lhs_theta(lhs_theta_fcts);

    vector<AbstractLocalOperator2D<T>* > lhs_ops1;
    vector<AbstractLocalOperator2D<T>* > lhs_ops2;
    vector<AbstractLocalOperator2D<T>* > bench;
    lhs_ops1.push_back(&localWeightLaplaceIdentityOp2D_0);
    lhs_ops1.push_back(&localWeightIdentityLaplaceOp2D_0);
    lhs_ops2.push_back(&localWeightLaplaceIdentityOp2D_1);
    lhs_ops2.push_back(&localWeightIdentityLaplaceOp2D_1);

    Flex_COp_2D     A1(lhs_ops1);
    Flex_COp_2D     A2(lhs_ops2);

    vector<Flex_COp_2D*>    ops_vec;
    ops_vec.push_back(&A1);
    ops_vec.push_back(&A2);
    ...
    AffineA_Rhs_2D	 	affineA_rhs(lhs_theta, lhs_ops1);
	...
	*/
    ...
    awgm_truth_parameters.max_its = 1e+02;

    is_parameters.adaptive_tol = true;
    is_parameters.init_tol = 0.001;
    is_parameters.res_reduction = 0.1;
    is_parameters.max_its       = 2e+02;
...
/*
    ...
    rb_base.greedy_params.tol           = 1e-02;
    rb_base.greedy_params.deter_rate    = 1./3.;
    rb_base.set_tol(1e-02*1e-02*1e-01);
   ...
   */
    ...
    //rb_base.read_rieszrepresentors("training_data_stempel/representors", 12);
    //rb_base.read_basisfunctions("training_data_stempel/bf", 12);
    ...
    //
        ...
        std::string     snapshot;
        std::string     plot;
        snapshot = "../3/training_data_stempel/snap/snap_1.txt";
        plot     = "plot_Arep1_12";
    ...
    long long j1max = 0, j2max = 0;
    long long card1max = 0, card2max = 0;
    long double suppmin = 100.;
    for (auto& lambda : u) {
        auto j1    = lambda.first.index1.j;
        auto j2    = lambda.first.index2.j;
        if (j1==0) continue;
        auto card1 = basis_x.cardJ(j1+1);
        auto card2 = basis_y.cardJ(j2+1);
        long double supp1 = basis_x.psi.support(j1, lambda.first.index1.k).length();
        long double supp2 = basis_y.psi.support(j2, lambda.first.index2.k).length();
        long double supp  = supp1*supp2;
        if (supp<=0) {
            std::cout << "j1 = " << j1 << std::endl;
            std::cout << "j2 = " << j2 << std::endl;
            std::cout << "k1 = " << lambda.first.index1.k << std::endl;
            std::cout << "k2 = " << lambda.first.index2.k << std::endl;
            std::cout << "supp1 = " << supp1 << std::endl;
            std::cout << "supp2 = " << supp2 << std::endl;
        }

        j1max      = (j1>j1max) ? j1 : j1max;
        card1max   = (card1>card1max) ? card1 : card1max;
        j2max      = (j2>j2max) ? j2 : j2max;
        card2max   = (card2>card2max) ? card2 : card2max;
        suppmin    = (supp<suppmin) ? supp : suppmin;
    }

    std::cout << "j1max    = " << j1max    << std::endl;
    std::cout << "j2max    = " << j2max    << std::endl;
    std::cout << "card1max = " << card1max << std::endl;
    std::cout << "card2max = " << card2max << std::endl;
    std::cout << "suppmin  = " << suppmin  << std::endl;
    std::cout << basis_x.psi.support(10, 5).length() << std::endl;
    std::cout << basis_y.psi.support(10, 5).length() << std::endl;


/*
        rb_base.rb_truth.access_solver().remove_preconditioner(u);
        precon _p;
        plot2D(basis2d, u, _p,
                                 zero,
                                 0., 1.,
...
/*
    ...
    DataType& bf = rb_base.rb_basisfunctions[9];
    ...
    DataType Au;
    sample_Au(rb_base.rb_truth.access_solver().get_testbasis(),
              _Lambda,
              (*rb_base.rb_truth.access_solver().get_lhs().get_localops()[1]),
              rb_base.rb_truth.access_solver().get_testprec(),
              bf,
              Au,
              1e-04,
              false);
    saveCoeffVector2D(bf, rb_base.rb_truth.access_solver().get_testbasis(),
                      "Autest.txt");

    exit(1);*/


    //rb_base.train_Greedy(12);
