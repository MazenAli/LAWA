...
L2_H1_error(Basis2D& basis2d, const Coefficients<Lexicographical,T,Index2D> &u, Prec& P,
            T (*u_ref)(T,T), T (*dx_u_ref)(T,T), T a_t, T b_t, FLENS_DEFAULT_INDEXTYPE n_t, T a_x, T b_x, FLENS_DEFAULT_INDEXTYPE n_x)
{
  ...
  T h_t = (b_t - a_t)/(n_t-1);
  T h_x = (b_x - a_x)/(n_x-1);
  for (FLENS_DEFAULT_INDEXTYPE i = 0; i < n_t; ++i) {
      T H1_error = 0.;
      T t = a_t + i*h_t;
      for (FLENS_DEFAULT_INDEXTYPE j = 0; j < n_x; ++j) {
          T x = a_x + j*h_x;
          ...
          for (coeff_it it = u.begin(); it != u.end(); ++it) {
              ...
              FLENS_DEFAULT_INDEXTYPE j_t = (*it).first.index1.j, k_t = (*it).first.index1.k;
              FLENS_DEFAULT_INDEXTYPE j_x = (*it).first.index2.j, k_x = (*it).first.index2.k;
