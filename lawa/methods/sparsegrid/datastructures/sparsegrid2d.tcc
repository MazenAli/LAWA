...
SparseGrid2D<T, IndexOneD, Basis2D, BilinearForm, RHS>::solve_cg(T &energynorm)
{
    ...
    FLENS_DEFAULT_INDEXTYPE NumOfIterations = lawa::cg(A,u,b,1e-14,1000);
    Au = A*u;
...
{
    assert((FLENS_DEFAULT_INDEXTYPE)(u.length())==(FLENS_DEFAULT_INDEXTYPE)(Lambda.size()));
    ...
    for (const_set_it it=Lambda.begin(); it!=Lambda.end(); ++it,++count) {
        FLENS_DEFAULT_INDEXTYPE j_x = (*it).index1.j,  k_x = (*it).index1.k;
        FLENS_DEFAULT_INDEXTYPE j_y = (*it).index2.j,  k_y = (*it).index2.k;
...
SparseGrid2D<T, IndexOneD, Basis2D, BilinearForm, RHS>::setupIndexSet()
{
    ...
    FLENS_DEFAULT_INDEXTYPE j0_x = basis.first.mra.j0;
    FLENS_DEFAULT_INDEXTYPE j0_y = basis.second.mra.j0;

    for (FLENS_DEFAULT_INDEXTYPE k_x=basis.first.mra.rangeI(j0_x).firstIndex();
             k_x<=basis.first.mra.rangeI(j0_x).lastIndex(); ++k_x) {
        for (FLENS_DEFAULT_INDEXTYPE k_y=basis.second.mra.rangeI(j0_y).firstIndex();
    ...
    for (FLENS_DEFAULT_INDEXTYPE k_x=basis.first.mra.rangeI(j0_x).firstIndex();
             k_x<=basis.first.mra.rangeI(j0_x).lastIndex(); ++k_x) {
        for (FLENS_DEFAULT_INDEXTYPE j_y=j0_y; j_y<=J+j0_y; ++j_y) {
            for (FLENS_DEFAULT_INDEXTYPE k_y=basis.second.rangeJ(j_y).firstIndex();
    ...
    for (FLENS_DEFAULT_INDEXTYPE j_x=j0_x; j_x<=J+j0_x; ++j_x) {
        for (FLENS_DEFAULT_INDEXTYPE k_x=basis.first.rangeJ(j_x).firstIndex();
