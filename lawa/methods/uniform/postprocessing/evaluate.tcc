        ...
        const FLENS_DEFAULT_INDEXTYPE deriv_y)
{
    ...
    const FLENS_DEFAULT_INDEXTYPE j0_x = basis.first.j0;
    const FLENS_DEFAULT_INDEXTYPE j0_y = basis.second.j0;   
    ...
    /* SF * SF */
    flens::Range<FLENS_DEFAULT_INDEXTYPE> Rx = basis.first.mra.rangeI(j0_x);
    flens::Range<FLENS_DEFAULT_INDEXTYPE> Ry = basis.second.mra.rangeI(j0_y);
    ...
    for(FLENS_DEFAULT_INDEXTYPE jy = j0_y; jy <= basis.J2_max(J_x, J_y, j0_x - 1) -1; ++jy){
    ...
    for(FLENS_DEFAULT_INDEXTYPE jx = j0_x; jx <= basis.J1_max(J_x, J_y, j0_y - 1) - 1; ++jx){
    ...
    /* W * W */
    for(FLENS_DEFAULT_INDEXTYPE jx = j0_x; jx <= basis.J1_max(J_x, J_y, j0_y)-1; ++jx){
        Rx = basis.first.rangeJ(jx);
        for(FLENS_DEFAULT_INDEXTYPE jy = j0_y; jy <= basis.J2_max(J_x, J_y, jx)-1; ++jy){
            Ry = basis.second.rangeJ(jy);
            for (FLENS_DEFAULT_INDEXTYPE kx = Rx.firstIndex(); kx <= Rx.lastIndex(); ++kx) {
                for(FLENS_DEFAULT_INDEXTYPE ky = Ry.firstIndex(); ky <= Ry.lastIndex(); ++ky){
