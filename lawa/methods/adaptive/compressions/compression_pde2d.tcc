                                             ...
                                             const IndexSet<Index2D> &LambdaRow)
{
    ...
    //FLENS_DEFAULT_INDEXTYPE level_thresh_bound = std::min(J,std::max(s_tilde_x,s_tilde_y));
...
template <typename T, typename Basis2D>
IndexSet<Index2D>
CompressionPDE2D<T,Basis2D>::SparsityPattern(const Index2D &lambda_col, FLENS_DEFAULT_INDEXTYPE jmin_x, FLENS_DEFAULT_INDEXTYPE jmin_y,
                                             FLENS_DEFAULT_INDEXTYPE s_tilde, FLENS_DEFAULT_INDEXTYPE deriv_x, FLENS_DEFAULT_INDEXTYPE deriv_y) {
    ...
    FLENS_DEFAULT_INDEXTYPE level_bound_x = round((1./factor_x)*s_tilde);
    FLENS_DEFAULT_INDEXTYPE level_bound_y = round((1./factor_y)*s_tilde);

    for (FLENS_DEFAULT_INDEXTYPE s_tilde_x=0; s_tilde_x <= level_bound_x; ++s_tilde_x) {
        for (FLENS_DEFAULT_INDEXTYPE s_tilde_y=0; s_tilde_y <= level_bound_y; ++s_tilde_y) {
                ...
                for (set1d_const_it lambda_x=Lambda_x.begin(); lambda_x!=Lambda_x.end(); ++lambda_x) {
                    for (set1d_const_it lambda_y=Lambda_y.begin(); lambda_y!=Lambda_y.end(); ++lambda_y) {
                        if (deriv_x!=0 || deriv_y!=0) {
                            ...
                            FLENS_DEFAULT_INDEXTYPE tmp =   std::max(lambda_col.index1.j,lambda_col.index2.j)
