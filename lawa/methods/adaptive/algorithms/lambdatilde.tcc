...
lambdaTilde1d_PDE(const Index1D &lambda, const Basis<T,Primal,R,CDF> &basis, 
                  FLENS_DEFAULT_INDEXTYPE s_tilde, FLENS_DEFAULT_INDEXTYPE jmin, FLENS_DEFAULT_INDEXTYPE jmax=100, bool update=false)
{
    ...
    FLENS_DEFAULT_INDEXTYPE d = psi.d;
    ...
    if (!update) {
        ...
        if (lambda.xtype == XBSpline) {
            ...
            FLENS_DEFAULT_INDEXTYPE kMin =  floor(pow2i<T>(j)*supp.l1 - support_refbspline.l2)-1;
            FLENS_DEFAULT_INDEXTYPE kMax =   ceil(pow2i<T>(j)*supp.l2 - support_refbspline.l1)+1;
            ...
            for (FLENS_DEFAULT_INDEXTYPE j_row=j; j_row<=std::min(j+s_tilde, jmax); ++j_row) {        // realization of matrix compression via level threshold
                ...
                if (j_row>=j+2) {
                    ...
                    for (FLENS_DEFAULT_INDEXTYPE i=singsupp.firstIndex(); i<=singsupp.lastIndex(); ++i) {
                        FLENS_DEFAULT_INDEXTYPE kMin = floor(pow2i<T>(j_row)*singsupp(i) - support_refwavelet.l2)-1;
                        FLENS_DEFAULT_INDEXTYPE kMax =  ceil(pow2i<T>(j_row)*singsupp(i) - support_refwavelet.l1)+1;
                    ...
                    FLENS_DEFAULT_INDEXTYPE kMin = floor(pow2i<T>(j_row)*supp.l1 - support_refwavelet.l2)-1;
                    FLENS_DEFAULT_INDEXTYPE kMax =  ceil(pow2i<T>(j_row)*supp.l2 - support_refwavelet.l1)+1;
            ...
            if (fabs(j - jmin) <= s_tilde) {
                FLENS_DEFAULT_INDEXTYPE kMin = floor( pow2i<T>(jmin)*supp.l1 - phi.support(0,0).l2)-1;
                FLENS_DEFAULT_INDEXTYPE kMax =  ceil( pow2i<T>(jmin)*supp.l2 - phi.support(0,0).l1)+1;
                for (FLENS_DEFAULT_INDEXTYPE k_row=kMin; k_row<=kMax; ++k_row) {
            ...
            // Inserting all indices corresponding to Wavelets with intersecting support using
            // a) local compactness  b) matrix compression  c) vanishing moments
            for (FLENS_DEFAULT_INDEXTYPE j_row=std::max(j-s_tilde,jmin); j_row<=std::min(j+s_tilde,jmax); ++j_row) {
                T Pow2i_Mjrow = pow2i<T>(-j_row);
                if (j_row>=j+1) {
...
/*
    ...
    // Therefore, we only have to add wavelets on level jmax.
        ...
        FLENS_DEFAULT_INDEXTYPE j = lambda.j, k = lambda.k;
        FLENS_DEFAULT_INDEXTYPE d = lambda.d, d_= lambda.d_;
        ...
        if (lambda.xtype == XBSpline) {
            ...
            if (jmax <= j+s_tilde) {
                ...
                for (FLENS_DEFAULT_INDEXTYPE i=singpts.firstIndex(); i<=singpts.lastIndex(); ++i) {
                    FLENS_DEFAULT_INDEXTYPE kMin =  ceil(pow2i(jmax)*singpts(i) - psi_row.support(0,0).l2);
                    FLENS_DEFAULT_INDEXTYPE kMax = floor(pow2i(jmax)*singpts(i) - psi_row.support(0,0).l1);
                    for (FLENS_DEFAULT_INDEXTYPE k_row=kMin; k_row<=kMax; ++k_row) {
            ...
            Wavelet<T,Primal,R> psi_row(d,d_);
            if (jmax <= j+s_tilde) {
                FLENS_DEFAULT_INDEXTYPE level_diff = 3;
                if (d == 2) level_diff = 2;
                if (d == 3) level_diff = 4;
                if (jmax >= j+level_diff) {                                                // level difference has to be large enough for vanishing entries due to vanishing moments
                    ...
                    FLENS_DEFAULT_INDEXTYPE kMin = ceil( pow2i(jmax)*supp.l1 - psi_row.support(0,0).l2);
                    FLENS_DEFAULT_INDEXTYPE kMax = floor(pow2i(jmax)*supp.l2 - psi_row.support(0,0).l1);
                    for (FLENS_DEFAULT_INDEXTYPE k_row=kMin; k_row<=kMax; ++k_row) {
                  ...
                  FLENS_DEFAULT_INDEXTYPE s_tilde, FLENS_DEFAULT_INDEXTYPE jmin, FLENS_DEFAULT_INDEXTYPE jmax=100, bool update=false)
{
    ...
    if (lambda.xtype == XBSpline) {
        ...
        // Inserting all indices corresponding to Bsplines with intersecting support using local compactness
        for (FLENS_DEFAULT_INDEXTYPE k_row=k-(d-1)*numSplines; k_row<=k+(d-1)*numSplines; ++k_row) {
            if (overlap(supp, phi.support(j,k_row)) > 0) {
        ...
        // Inserting all indices corresponding to Wavelets with intersecting support using
        // a) local compactness  b) matrix compression  c) vanishing moments
        for (FLENS_DEFAULT_INDEXTYPE j_row=j; j_row<=std::min(j+s_tilde, jmax); ++j_row) {
            T Pow2i_Mjrow = pow2i<T>(-j_row);

            if (j_row>=j+2) {
                flens::DenseVector<flens::Array<T> > singsupp = phi.singularSupport(j,k);
                for (FLENS_DEFAULT_INDEXTYPE i=singsupp.firstIndex(); i<=singsupp.lastIndex(); ++i) {
                    FLENS_DEFAULT_INDEXTYPE kMin = floor(pow2i<long double>(j_row)*singsupp(i) - max_support_refwavelet.l2)-1;
                    FLENS_DEFAULT_INDEXTYPE kMax =  ceil(pow2i<long double>(j_row)*singsupp(i) - max_support_refwavelet.l1)+1;
                    // todo: revise these loops and test!!
                    for (FLENS_DEFAULT_INDEXTYPE k_help=kMin; k_help<=kMax; ++k_help) {
                        for (FLENS_DEFAULT_INDEXTYPE k_row=(k_help-1)*numWavelets+1; k_row<=k_help*numWavelets; ++k_row) {
                            Support<T> supp_row = psi.support(j_row,k_row);
                            //std::cout << "LambdaTilde: Wavelet (" << j_row << ", k_row = " << k_row << "): " << supp_row << std::endl;
                            if (((overlap(supp_row, supp) > 0)) && (!(distance(singsupp,supp_row) >= 0 ))) {
                                //Attention: cast from FLENS_DEFAULT_INDEXTYPE to FLENS_DEFAULT_INDEXTYPE here if old index class is used!!
                                ret.insert(Index1D(j_row,k_row,XWavelet));
                            }
                        }
                    }
                ...
                FLENS_DEFAULT_INDEXTYPE kMin = floor(pow2i<long double>(j_row)*supp.l1 - max_support_refwavelet.l2)-1;
                FLENS_DEFAULT_INDEXTYPE kMax =  ceil(pow2i<long double>(j_row)*supp.l2 - max_support_refwavelet.l1)+1;
                // todo: revise these loops and test!!
                for (FLENS_DEFAULT_INDEXTYPE k_help=kMin; k_help<=kMax; ++k_help) {
                    for (FLENS_DEFAULT_INDEXTYPE k_row=(k_help-1)*numWavelets+1; k_row<=k_help*numWavelets; ++k_row) {
                        Support<T> supp_row = psi.support(j_row,k_row);
        ...
        // a) local compactness  b) matrix compression  c) vanishing moments
        if (fabs(j - jmin) <= s_tilde) {
            FLENS_DEFAULT_INDEXTYPE kMin = floor( pow2i<long double>(jmin)*supp.l1 - max_support_refbspline.l2);
            FLENS_DEFAULT_INDEXTYPE kMax =  ceil( pow2i<long double>(jmin)*supp.l2 - max_support_refbspline.l1);
            // todo: revise these loops and test!!
            for (FLENS_DEFAULT_INDEXTYPE k_help=kMin; k_help<=kMax; ++k_help) {
                for (FLENS_DEFAULT_INDEXTYPE k_row=(k_help-1)*numSplines+1; k_row<=k_help*numSplines; ++k_row) {
                    if (    (overlap(supp, phi.support(jmin,k_row)) > 0)
                         && (!(distance(supp,phi.singularSupport(jmin,k_row)) >= 0 )) )
                    {
        ...
        for (FLENS_DEFAULT_INDEXTYPE j_row=std::max(j-s_tilde,jmin); j_row<=std::min(j+s_tilde,jmax); ++j_row) {
            ...
            if (j_row>=j+4) {
                ...
                //cout << "LambdaTilde: Singular support psi_col_" << j << "," << k << " = " << singpts;
                for (FLENS_DEFAULT_INDEXTYPE i=singsupp.firstIndex(); i<=singsupp.lastIndex(); ++i) {
                    FLENS_DEFAULT_INDEXTYPE kMin = floor(pow2i<long double>(j_row)*singsupp(i) - max_support_refwavelet.l2);
                    FLENS_DEFAULT_INDEXTYPE kMax =  ceil(pow2i<long double>(j_row)*singsupp(i) - max_support_refwavelet.l1);
                    // todo: revise these loops and test!!
                    for (FLENS_DEFAULT_INDEXTYPE k_help=kMin; k_help<=kMax; ++k_help) {
                        for (FLENS_DEFAULT_INDEXTYPE k_row=(k_help-1)*numWavelets+1; k_row<=k_help*numWavelets; ++k_row) {
                            Support<T> supp_row = psi.support(j_row,k_row);
                  ...
                  FLENS_DEFAULT_INDEXTYPE s_tilde, FLENS_DEFAULT_INDEXTYPE jmin, FLENS_DEFAULT_INDEXTYPE jmax=100, bool update=false)
{
    ...
    FLENS_DEFAULT_INDEXTYPE numWavelets = (FLENS_DEFAULT_INDEXTYPE)std::max(basis._numLeftParts,basis._numInnerParts);
    ...
    if (lambda.xtype == XBSpline) {
        ...
        // Inserting all indices corresponding to Bsplines with intersecting support using local compactness
        for (FLENS_DEFAULT_INDEXTYPE k_row=basis.mra.rangeI(j).firstIndex(); k_row<=basis.mra.rangeI(j).lastIndex(); ++k_row) {
            if (overlap(supp, phi.support(j,k_row)) > 0) {
                //std::cout << "lambdaTilde: BSpline (" << j << ", " << k_row << "): " << phi.support(j,k_row) << std::endl;
                ret.insert(Index1D(j,k_row,XBSpline));
            }
        ...
        for (FLENS_DEFAULT_INDEXTYPE j_row=j; j_row<=std::min(j+s_tilde, jmax); ++j_row) {
            ...
            for (FLENS_DEFAULT_INDEXTYPE k_row=basis.rangeJL(j_row).firstIndex(); k_row<=basis.rangeJL(j_row).lastIndex(); ++k_row) {
                Support<T> supp_row = psi.support(j_row,k_row);
                //std::cout << "LambdaTilde: Wavelet (" << j_row << ", k_row = " << k_row << "): " << supp_row << std::endl;
            ...
            for (FLENS_DEFAULT_INDEXTYPE k_row=basis.rangeJR(j_row).firstIndex(); k_row<=basis.rangeJR(j_row).lastIndex(); ++k_row) {
                if (j_row < 0) {
            ...
            if (j_row>=j+4) {
                flens::DenseVector<flens::Array<T> > singsupp = phi.singularSupport(j,k);
                for (FLENS_DEFAULT_INDEXTYPE i=singsupp.firstIndex(); i<=singsupp.lastIndex(); ++i) {
                    FLENS_DEFAULT_INDEXTYPE kMin = floor(pow2i<FLENS_DEFAULT_INDEXTYPE>(j_row)*singsupp(i) - max_support.l2)-1;
                    FLENS_DEFAULT_INDEXTYPE kMax =  ceil(pow2i<FLENS_DEFAULT_INDEXTYPE>(j_row)*singsupp(i) - max_support.l1)+1;
                    FLENS_DEFAULT_INDEXTYPE k_first = std::max((kMin-1)*numWavelets+1, (FLENS_DEFAULT_INDEXTYPE)basis.rangeJI(j_row).firstIndex());
                    FLENS_DEFAULT_INDEXTYPE k_last  = std::min(kMax*numWavelets,       (FLENS_DEFAULT_INDEXTYPE)basis.rangeJI(j_row).lastIndex());
                    for (FLENS_DEFAULT_INDEXTYPE k_row=k_first; k_row<=k_last; ++k_row) {
                        Support<T> supp_row = psi.support(j_row,k_row);
                        //std::cout << "LambdaTilde: Wavelet (" << j_row << ", k_row = " << k_row << "): " << supp_row << std::endl;
                        if (((overlap(supp_row, supp) > 0)) && (!(distance(singsupp,supp_row) >= 0 ))) {
                            //Attention: cast from FLENS_DEFAULT_INDEXTYPE to FLENS_DEFAULT_INDEXTYPE here if old index class is used!!
                ...
                for (FLENS_DEFAULT_INDEXTYPE j_row=j; j_row<=j+3; ++j_row) {
                    for (FLENS_DEFAULT_INDEXTYPE k_row=basis.rangeJI(j_row).firstIndex(); k_row<=basis.rangeJI(j_row).lastIndex(); ++k_row) {
                        Support<T> supp_row = psi.support(j_row,k_row);
                        if (overlap(supp, supp_row) > 0)  {
                            ret.insert(Index1D(j_row,k_row,XWavelet));
                        }
        ...
        // a) local compactness  b) matrix compression  c) vanishing moments
        if (fabs(j - jmin) <= s_tilde) {
            for (FLENS_DEFAULT_INDEXTYPE k_row=basis.mra.rangeI(jmin).firstIndex(); k_row<=basis.mra.rangeI(jmin).lastIndex(); ++k_row) {
                if (overlap(supp, phi.support(jmin,k_row)) > 0) {
        ...
        for (FLENS_DEFAULT_INDEXTYPE j_row=std::max(j-s_tilde,jmin); j_row<=std::min(j+s_tilde,jmax); ++j_row) {
            ...
            if (j_row>=j+4) {
                ...
                //cout << "LambdaTilde: Singular support psi_col_" << j << "," << k << " = " << singpts;
                for (FLENS_DEFAULT_INDEXTYPE i=singsupp.firstIndex(); i<=singsupp.lastIndex(); ++i) {
                    FLENS_DEFAULT_INDEXTYPE kMin = floor(pow2i<long double>(j_row)*singsupp(i) - max_support.l2);
                    FLENS_DEFAULT_INDEXTYPE kMax =  ceil(pow2i<long double>(j_row)*singsupp(i) - max_support.l1);
                    FLENS_DEFAULT_INDEXTYPE k_first = std::max((kMin-1)*numWavelets,(FLENS_DEFAULT_INDEXTYPE)basis.rangeJI(j_row).firstIndex());
                    FLENS_DEFAULT_INDEXTYPE k_last  = std::min(kMax*numWavelets,    (FLENS_DEFAULT_INDEXTYPE)basis.rangeJI(j_row).lastIndex());
                ...
                FLENS_DEFAULT_INDEXTYPE kMin = floor(pow2i<long double>(j_row)*supp.l1 - max_support.l2)-1;
                FLENS_DEFAULT_INDEXTYPE kMax =  ceil(pow2i<long double>(j_row)*supp.l2 - max_support.l1)+1;
                FLENS_DEFAULT_INDEXTYPE k_first = std::max((kMin-1)*numWavelets,(FLENS_DEFAULT_INDEXTYPE)basis.rangeJI(j_row).firstIndex());
                FLENS_DEFAULT_INDEXTYPE k_last  = std::min(kMax*numWavelets,    (FLENS_DEFAULT_INDEXTYPE)basis.rangeJI(j_row).lastIndex());
                  ...
                  FLENS_DEFAULT_INDEXTYPE /*s_tilde*/, FLENS_DEFAULT_INDEXTYPE jmin, FLENS_DEFAULT_INDEXTYPE jmax=100, bool update=false)
{
    ...
    jmax = std::min(jmax,(FLENS_DEFAULT_INDEXTYPE)JMAX);
    ...
    if (lambda.xtype == XBSpline) {
        ...
        FLENS_DEFAULT_INDEXTYPE kMin = floor( pow2i<long double>(j)*supp.l1 - max_support_refbspline.l2) - 1;
        FLENS_DEFAULT_INDEXTYPE kMax =  ceil( pow2i<long double>(j)*supp.l2 - max_support_refbspline.l1) + 1;
        ...
        if (j==jmin) {
            ...
            FLENS_DEFAULT_INDEXTYPE kMin = floor( pow2i<long double>(jmin)*supp.l1 - max_support_refbspline.l2) - 1;
            FLENS_DEFAULT_INDEXTYPE kMax =  ceil( pow2i<long double>(jmin)*supp.l2 - max_support_refbspline.l1) + 1;
        ...
        // Inserting all indices corresponding to Wavelets with intersecting support using
        // a) local compactness  b) matrix compression  c) vanishing moments
        for (FLENS_DEFAULT_INDEXTYPE j_row=std::max(j-1,jmin); j_row<=std::min(j+1,jmax); ++j_row) {

            FLENS_DEFAULT_INDEXTYPE kMin = floor( pow2i<long double>(j_row)*supp.l1 - max_support_refwavelet.l2) / 2 - 1;
            FLENS_DEFAULT_INDEXTYPE kMax =  ceil( pow2i<long double>(j_row)*supp.l2 - max_support_refwavelet.l1) / 2 + 1;
...
lambdaTilde1d_PDE(const Index1D &lambda, const Basis<T,Primal,Periodic,CDF> &basis, 
                  FLENS_DEFAULT_INDEXTYPE s_tilde, FLENS_DEFAULT_INDEXTYPE jmin, FLENS_DEFAULT_INDEXTYPE jmax, bool update)
{
    ...
    if (!update) {
        ...
        if (lambda.xtype == XBSpline) {
            ...
            FLENS_DEFAULT_INDEXTYPE kMax =   ceil(pow2i<T>(j)*supp.l2 - support_refbspline.l1)+1;
            for (FLENS_DEFAULT_INDEXTYPE k_row=kMin; k_row<=kMax; ++k_row) {
                FLENS_DEFAULT_INDEXTYPE k_row_per = k_row;
            ...
            // a) local compactness  b) matrix compression  c) vanishing moments
            if (fabs(j - jmin) <= s_tilde) {
                FLENS_DEFAULT_INDEXTYPE kMin = floor(pow2i<T>(j)*supp.l1 - support_refbspline.l2);
                FLENS_DEFAULT_INDEXTYPE kMax =  ceil(pow2i<T>(j)*supp.l2 - support_refbspline.l1);
                for (FLENS_DEFAULT_INDEXTYPE k_row=kMin; k_row<=kMax; ++k_row) {
                    FLENS_DEFAULT_INDEXTYPE k_row_per = k_row;
                  ...
                  FLENS_DEFAULT_INDEXTYPE s_tilde, FLENS_DEFAULT_INDEXTYPE jmin, FLENS_DEFAULT_INDEXTYPE jmax, bool /*update*/) 
{
        ...
        if (lambda.xtype==XBSpline) {
            ...
            //Adding B-Splines
            FLENS_DEFAULT_INDEXTYPE kMin = basis.mra.rangeI(jmin).firstIndex(), kMax = basis.mra.rangeI(jmin).lastIndex();
            FLENS_DEFAULT_INDEXTYPE kStart = min(max(iceil<FLENS_DEFAULT_INDEXTYPE>(supp_col.l1 * pow2i<T>(jmin)),kMin), kMax);
            ...
            FLENS_DEFAULT_INDEXTYPE kEnd = max(min(ifloor(supp_col.l2 * pow2i<T>(jmin)),kMax), kMin);
            ...
            for (FLENS_DEFAULT_INDEXTYPE k_row=kStart; k_row<=kEnd; ++k_row) {
                ret.insert(Index1D(jmin,k_row,XBSpline));
            }

            //Adding Wavelets
            for (FLENS_DEFAULT_INDEXTYPE j_row=jmin; j_row<=min(jmin+s_tilde, jmax); ++j_row) {

                FLENS_DEFAULT_INDEXTYPE kMin = basis.rangeJ(j_row).firstIndex(), kMax = basis.rangeJ(j_row).lastIndex();
                FLENS_DEFAULT_INDEXTYPE kStart = min(max(iceil<FLENS_DEFAULT_INDEXTYPE>(supp_col.l1 * pow2i<T>(j_row)), kMin), kMax);
                ...
                FLENS_DEFAULT_INDEXTYPE kEnd = max(min(ifloor(supp_col.l2 * pow2i<T>(j_row)), kMax), kMin);
                ...
                for (FLENS_DEFAULT_INDEXTYPE k_row=kStart; k_row<=kEnd; k_row++) {
                    flens::Range<FLENS_DEFAULT_INDEXTYPE> rangeL = basis.rangeJL(j_row);
                    flens::Range<FLENS_DEFAULT_INDEXTYPE> rangeR = basis.rangeJR(j_row);
            ...
            //Adding Wavelets
            for (FLENS_DEFAULT_INDEXTYPE j_row=max(j-s_tilde,jmin); j_row<=min(j+s_tilde,jmax); ++j_row) {
...
lambdaTilde1d_PDE_WO_XBSpline(const Index1D &lambda, const Basis<T,Primal,R,CDF> &basis, 
                              FLENS_DEFAULT_INDEXTYPE s_tilde, FLENS_DEFAULT_INDEXTYPE jmin, FLENS_DEFAULT_INDEXTYPE jmax)
{
    const Wavelet<T,Primal,R,CDF> psi = basis.psi;
    FLENS_DEFAULT_INDEXTYPE j = lambda.j, k = lambda.k;
    IndexSet<Index1D> ret;
...
lambdaTilde1d_WeightedPDE(const Index1D &lambda, const Basis<T,Primal,Interval,Cons> &basis,
                          FLENS_DEFAULT_INDEXTYPE s_tilde_level, FLENS_DEFAULT_INDEXTYPE jmin, FLENS_DEFAULT_INDEXTYPE jmax, FLENS_DEFAULT_INDEXTYPE s_tilde_singsupp)
{
    using std::min;
    using std::max;

    if (s_tilde_singsupp==-1) {
        s_tilde_singsupp = (FLENS_DEFAULT_INDEXTYPE)((basis.d-1.5)*s_tilde_level/(1.+basis.d_))+1;
    }
    ...
    if (lambda.xtype==XBSpline) {
        ...
        //Adding Wavelets
        for (FLENS_DEFAULT_INDEXTYPE j_row=jmin; j_row<=min(jmin+s_tilde_level, jmax); ++j_row) {
        ...
        //Adding Wavelets
        for (FLENS_DEFAULT_INDEXTYPE j_row=max(j-s_tilde_level,jmin); j_row<=min(j+s_tilde_level,jmax); ++j_row) {
                          ...
                          FLENS_DEFAULT_INDEXTYPE s_tilde_level, FLENS_DEFAULT_INDEXTYPE jmin, FLENS_DEFAULT_INDEXTYPE jmax, FLENS_DEFAULT_INDEXTYPE s_tilde_singsupp)
{
    ...
    if (s_tilde_singsupp==-1) {
        s_tilde_singsupp = (FLENS_DEFAULT_INDEXTYPE)((basis.d-1.5)*s_tilde_level/(1.+basis.d_))+4;
    }
    ...
    if (lambda.xtype == XBSpline) {
        ...
        for (FLENS_DEFAULT_INDEXTYPE j_row=j; j_row<=std::min(j+s_tilde_level, jmax); ++j_row) {        // realization of matrix compression via level threshold
            T Pow2i_Mjrow = pow2i<T>(-j_row);
        ...
        // Inserting all indices corresponding to Wavelets with intersecting support using
        // a) local compactness  b) matrix compression  c) vanishing moments for level diff >= s_tilde_singsupp
        for (FLENS_DEFAULT_INDEXTYPE j_row=std::max(j-s_tilde_level,jmin); j_row<=std::min(j+s_tilde_level,jmax); ++j_row) {
            T Pow2i_Mjrow = pow2i<T>(-j_row);
            if (j_row>j+s_tilde_singsupp) {
                flens::DenseVector<flens::Array<T> > singsupp = psi.optim_singularSupport(j,k);
                //cout << "LambdaTilde: Singular support psi_col_" << j << "," << k << " = " << singpts;
