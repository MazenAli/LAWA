...
void
extendMultiTree(const Basis &basis, const Coefficients<Lexicographical,T,Index3D>  &v,
                Coefficients<Lexicographical,T,Index3D>  &C_v, FLENS_DEFAULT_INDEXTYPE coordDirec)
{
    typedef typename Coefficients<Lexicographical,T,Index3D>::const_iterator const_coeff3d_it;
    typedef IndexSet<Index1D>::const_iterator                                const_set1d_it;
...
extendMultiTreeAtBoundary(const Basis &basis, const Coefficients<Lexicographical,T,Index2D>  &v,
                          Coefficients<Lexicographical,T,Index2D>  &C_v, FLENS_DEFAULT_INDEXTYPE J, bool sparsetree)
{
    typedef IndexSet<Index1D>::const_iterator                                const_set1d_it;

    IndexSet<Index1D> LambdaB_x1;
    for (FLENS_DEFAULT_INDEXTYPE k= basis.first.mra.rangeI(basis.first.j0).firstIndex();
             k<=basis.first.mra.rangeI(basis.first.j0).lastIndex(); ++k) {
        LambdaB_x1.insert(Index1D(basis.first.j0,k,XBSpline));
    }
    for (FLENS_DEFAULT_INDEXTYPE j=basis.first.j0; j<=J; ++j) {
        for (FLENS_DEFAULT_INDEXTYPE k=basis.first.rangeJL(j).firstIndex(); k<=basis.first.rangeJL(j).lastIndex(); ++k) {
            LambdaB_x1.insert(Index1D(j,k,XWavelet));
        }
        for (FLENS_DEFAULT_INDEXTYPE k=basis.first.rangeJR(j).firstIndex(); k<=basis.first.rangeJR(j).lastIndex(); ++k) {
            LambdaB_x1.insert(Index1D(j,k,XWavelet));
        }
    }
    IndexSet<Index1D> LambdaB_x2;
    for (FLENS_DEFAULT_INDEXTYPE k= basis.second.mra.rangeI(basis.second.j0).firstIndex();
             k<=basis.second.mra.rangeI(basis.second.j0).lastIndex(); ++k) {
        LambdaB_x2.insert(Index1D(basis.second.j0,k,XBSpline));
    }
    for (FLENS_DEFAULT_INDEXTYPE j=basis.second.j0; j<=J; ++j) {
        for (FLENS_DEFAULT_INDEXTYPE k=basis.second.rangeJL(j).firstIndex(); k<=basis.second.rangeJL(j).lastIndex(); ++k) {
            LambdaB_x2.insert(Index1D(j,k,XWavelet));
        }
        for (FLENS_DEFAULT_INDEXTYPE k=basis.second.rangeJR(j).firstIndex(); k<=basis.second.rangeJR(j).lastIndex(); ++k) {
            LambdaB_x2.insert(Index1D(j,k,XWavelet));
        }
    }
...
extendMultiTreeAtBoundary(const Basis &basis, const Coefficients<Lexicographical,T,Index3D>  &v,
                          Coefficients<Lexicographical,T,Index3D>  &C_v, FLENS_DEFAULT_INDEXTYPE J, bool sparsetree)
{
    typedef IndexSet<Index1D>::const_iterator                                const_set1d_it;
    ...
    for (FLENS_DEFAULT_INDEXTYPE k= basis.third.mra.rangeI(basis.third.j0).firstIndex();
             k<=basis.third.mra.rangeI(basis.third.j0).lastIndex(); ++k) {
        LambdaB_x3.insert(Index1D(basis.third.j0,k,XBSpline));
    }
    for (FLENS_DEFAULT_INDEXTYPE j=basis.first.j0; j<=J; ++j) {
        for (FLENS_DEFAULT_INDEXTYPE k=basis.third.rangeJL(j).firstIndex(); k<=basis.third.rangeJL(j).lastIndex(); ++k) {
            LambdaB_x3.insert(Index1D(j,k,XWavelet));
        }
        for (FLENS_DEFAULT_INDEXTYPE k=basis.third.rangeJR(j).firstIndex(); k<=basis.third.rangeJR(j).lastIndex(); ++k) {
            LambdaB_x3.insert(Index1D(j,k,XWavelet));
        }
    }
    for (const_set1d_it it_x1=LambdaB_x1.begin(); it_x1!=LambdaB_x1.end(); ++it_x1) {
                  ...
                  Coefficients<Lexicographical,T,Index1D>  &v, bool sparsetree)
{
    ...
    FLENS_DEFAULT_INDEXTYPE  j = index1d.j;
    FLENS_DEFAULT_INDEXTYPE k = index1d.k;

    Support<typename Basis::T> supp = basis.generator(index1d.xtype).support(j,k);

    FLENS_DEFAULT_INDEXTYPE new_j = 0;
    FLENS_DEFAULT_INDEXTYPE new_k_first = 0, new_k_last = 0;
    ...
    if (checkPredecessors) {
        if (!sparsetree) {
            for (FLENS_DEFAULT_INDEXTYPE new_k=new_k_first; new_k<=new_k_last; ++new_k) {
                  ...
                  IndexSet<Index1D>  &v, bool sparsetree)
{
    ...
    if (checkPredecessors) {
        if (!sparsetree) {
            ...
            FLENS_DEFAULT_INDEXTYPE covering_k = new_k_last;
            for (FLENS_DEFAULT_INDEXTYPE new_k=new_k_first; new_k<=new_k_last; ++new_k) {
                Support<typename Basis::T> covered_supp = basis.generator(new_type).support(new_j,new_k);
                if (covered_supp.l1<=supp.l1 && covered_supp.l2>=supp.l2) {
    ...
    if (checkPredecessors) {
        if (!sparsetree) {
        	if(new_k_first < new_k_last){
        		...
        		FLENS_DEFAULT_INDEXTYPE lastIndex = 0;
        		FLENS_DEFAULT_INDEXTYPE firstIndex = 0;
                ...
                for (FLENS_DEFAULT_INDEXTYPE new_k=new_k_first; new_k <= lastIndex ; ++new_k) {
                    PeriodicSupport<typename Basis::T> new_supp = basis.generator(new_type).support(new_j,new_k);
                    if (overlap(supp,new_supp)>0) {
                        Index1D new_index1d(new_j,new_k,new_type);
                        if (v.find(new_index1d)==v.end()) completeMultiTree(basis,new_index1d,v);
                ...
                for (FLENS_DEFAULT_INDEXTYPE new_k=firstIndex; new_k <=new_k_last ; ++new_k) {
                    PeriodicSupport<typename Basis::T> new_supp = basis.generator(new_type).support(new_j,new_k);
                    if (overlap(supp,new_supp)>0) {
                        Index1D new_index1d(new_j,new_k,new_type);
                        if (v.find(new_index1d)==v.end()) completeMultiTree(basis,new_index1d,v);
            		...
            		if(new_k_first < new_k_last){
						for (FLENS_DEFAULT_INDEXTYPE new_k=new_k_first; new_k<=new_k_last; ++new_k) {
	                        ...
	                        if (std::max(new_supp.l1, new_supp.li2) >= std::max(supp.l1, supp.li2)) {
	                            ...
	                            bool wrapped = false;
	                            if (std::max(new_supp.l1, new_supp.li2) > std::max(supp.l1, supp.li2)) {
	                        		FLENS_DEFAULT_INDEXTYPE left_k = new_k-1 >= firstIndex ? new_k - 1 : lastIndex;
...
completeMultiTree(const Basis &basis, const Index2D &index2d,
                  Coefficients<Lexicographical,T,Index2D>  &v, FLENS_DEFAULT_INDEXTYPE coordDirec, bool sparsetree,
                  bool isAlreadyMultiTree)
{
    FLENS_DEFAULT_INDEXTYPE j0_x = basis.first.j0;
    FLENS_DEFAULT_INDEXTYPE j0_y = basis.second.j0;
    ...
    FLENS_DEFAULT_INDEXTYPE  j_x = index_x.j, j_y = index_y.j;
    FLENS_DEFAULT_INDEXTYPE k_x = index_x.k, k_y = index_y.k;

    if (coordDirec==0 || coordDirec==1) {
        Support<typename Basis::T> supp_x = basis.first.generator(index_x.xtype).support(j_x,k_x);
        //check x-direction
        FLENS_DEFAULT_INDEXTYPE new_j_x = 0;
        FLENS_DEFAULT_INDEXTYPE new_k_x_first = 0, new_k_x_last = 0;
        bool checkPredecessors=true;
        ...
        if (checkPredecessors) {
            if (!sparsetree) {
                for (FLENS_DEFAULT_INDEXTYPE new_k_x=new_k_x_first; new_k_x<=new_k_x_last; ++new_k_x) {
                ...
                bool foundPredecessor = false;
                bool foundCoveredSupp = false;
                FLENS_DEFAULT_INDEXTYPE covering_k = new_k_x_last;
                for (FLENS_DEFAULT_INDEXTYPE new_k_x=new_k_x_first; new_k_x<=new_k_x_last; ++new_k_x) {
    ...
    if (coordDirec==0 || coordDirec==2) {
        ...
        //check y-direction
        bool checkPredecessors=true;
        FLENS_DEFAULT_INDEXTYPE new_j_y = 0;
        FLENS_DEFAULT_INDEXTYPE new_k_y_first = 0, new_k_y_last = 0;
        ...
        if (checkPredecessors) {
            if (!sparsetree) {
                for (FLENS_DEFAULT_INDEXTYPE new_k_y=new_k_y_first; new_k_y<=new_k_y_last; ++new_k_y) {
                ...
                bool foundPredecessor = false;
                bool foundCoveredSupp = false;
                FLENS_DEFAULT_INDEXTYPE covering_k = new_k_y_last;
                for (FLENS_DEFAULT_INDEXTYPE new_k_y=new_k_y_first; new_k_y<=new_k_y_last; ++new_k_y) {
		...
		if (checkPredecessors) {
			if (!sparsetree) {
				if(new_k_x_first < new_k_x_last){
					...
					for (FLENS_DEFAULT_INDEXTYPE new_k_x=firstIndex; new_k_x<=new_k_x_last; ++new_k_x) {
						PeriodicSupport<typename Basis::T> new_supp_x = basis.first.generator(new_type_x).support(new_j_x,new_k_x);
						if (overlap(supp_x,new_supp_x)>0) {
							Index2D new_index2d(Index1D(new_j_x,new_k_x,new_type_x),index_y);
							if (v.find(new_index2d)==v.end()) completeMultiTree(basis,new_index2d,v,coordDirec);
					...
					for (FLENS_DEFAULT_INDEXTYPE new_k_x=new_k_x_first; new_k_x<=lastIndex; ++new_k_x) {
						PeriodicSupport<typename Basis::T> new_supp_x = basis.first.generator(new_type_x).support(new_j_x,new_k_x);
						if (overlap(supp_x,new_supp_x)>0) {
							Index2D new_index2d(Index1D(new_j_x,new_k_x,new_type_x),index_y);
							if (v.find(new_index2d)==v.end()) completeMultiTree(basis,new_index2d,v,coordDirec);
						...
						if(new_k_x_first < new_k_x_last){
							for (FLENS_DEFAULT_INDEXTYPE new_k_x=new_k_x_first; new_k_x<=new_k_x_last; ++new_k_x) {
								...
								if (std::max(new_supp_x.l1, new_supp_x.li2) >= std::max(supp_x.l1, supp_x.li2)) {
									...
									bool wrapped = false;
									if (std::max(new_supp_x.l1, new_supp_x.li2) > std::max(supp_x.l1, supp_x.li2)) {
										FLENS_DEFAULT_INDEXTYPE left_k_x = new_k_x-1 >= firstIndex ? new_k_x - 1 : lastIndex;
		...
		if (checkPredecessors) {
			if (!sparsetree) {
				...
				if(new_k_y_first < new_k_y_last){
					...
					for (FLENS_DEFAULT_INDEXTYPE new_k_y=firstIndex; new_k_y<=new_k_y_last; ++new_k_y) {
						PeriodicSupport<typename Basis::T> new_supp_y = basis.second.generator(new_type_y).support(new_j_y,new_k_y);
						if (overlap(supp_y,new_supp_y)>0) {
							Index2D new_index2d(index_x,Index1D(new_j_y,new_k_y,new_type_y));
							if (v.find(new_index2d)==v.end()) completeMultiTree(basis,new_index2d,v,coordDirec);
					...
					for (FLENS_DEFAULT_INDEXTYPE new_k_y=new_k_y_first; new_k_y<=lastIndex; ++new_k_y) {
						PeriodicSupport<typename Basis::T> new_supp_y = basis.second.generator(new_type_y).support(new_j_y,new_k_y);
						if (overlap(supp_y,new_supp_y)>0) {
							Index2D new_index2d(index_x,Index1D(new_j_y,new_k_y,new_type_y));
							if (v.find(new_index2d)==v.end()) completeMultiTree(basis,new_index2d,v,coordDirec);
						...
						if(new_k_y_first < new_k_y_last){
							for (FLENS_DEFAULT_INDEXTYPE new_k_y=new_k_y_first; new_k_y<=new_k_y_last; ++new_k_y) {
								...
								if (std::max(new_supp_y.l1, new_supp_y.li2) >= std::max(supp_y.l1, supp_y.li2)) {
									...
									bool wrapped = false;
									if (std::max(new_supp_y.l1, new_supp_y.li2) > std::max(supp_y.l1, supp_y.li2)) {
										FLENS_DEFAULT_INDEXTYPE left_k_y = new_k_y-1 >= firstIndex ? new_k_y - 1 : lastIndex;
...
completeMultiTree(const Basis &basis, const Index2D &index2d,
                  ...
                  IndexSet<Index2D>& diff_v, FLENS_DEFAULT_INDEXTYPE coordDirec, bool sparsetree)
{
    FLENS_DEFAULT_INDEXTYPE j0_x = basis.first.j0;
    FLENS_DEFAULT_INDEXTYPE j0_y = basis.second.j0;
...
completeMultiTree(const Basis &basis, const Index3D &index3d,
                  Coefficients<Lexicographical,T,Index3D>  &v, FLENS_DEFAULT_INDEXTYPE coordDirec, bool sparsetree)
{
	assert(Basis::Domain != Periodic);

    FLENS_DEFAULT_INDEXTYPE j0_x = basis.first.j0;
    FLENS_DEFAULT_INDEXTYPE j0_y = basis.second.j0;
    FLENS_DEFAULT_INDEXTYPE j0_z = basis.third.j0;
    ...
    FLENS_DEFAULT_INDEXTYPE  j_x = index_x.j, j_y = index_y.j, j_z = index_z.j;
    FLENS_DEFAULT_INDEXTYPE k_x = index_x.k, k_y = index_y.k, k_z = index_z.k;
    ...
    if (coordDirec== 0 || coordDirec==3) {
        ...
        //check z-direction
        bool checkPredecessors=true;
        FLENS_DEFAULT_INDEXTYPE new_j_z = 0;
        FLENS_DEFAULT_INDEXTYPE new_k_z_first = 0, new_k_z_last = 0;
        ...
        if (checkPredecessors) {
            if (!sparsetree) {
                for (FLENS_DEFAULT_INDEXTYPE new_k_z=new_k_z_first; new_k_z<=new_k_z_last; ++new_k_z) {
...
getSparseGridVector(const Basis &basis, Coefficients<Lexicographical,T,Index2D> &v, FLENS_DEFAULT_INDEXTYPE j, T gamma)
{
    FLENS_DEFAULT_INDEXTYPE j0_x = basis.first.j0;
    FLENS_DEFAULT_INDEXTYPE j0_y = basis.second.j0;
    for (FLENS_DEFAULT_INDEXTYPE k1=basis.first.mra.rangeI(j0_x).firstIndex(); k1<=basis.first.mra.rangeI(j0_x).lastIndex(); ++k1) {
        for (FLENS_DEFAULT_INDEXTYPE k2=basis.second.mra.rangeI(j0_y).firstIndex(); k2<=basis.second.mra.rangeI(j0_y).lastIndex(); ++k2) {
            Index1D row(j0_x,k1,XBSpline);
            Index1D col(j0_y,k2,XBSpline);
            v[Index2D(row,col)] = 0.;
        }
        for (FLENS_DEFAULT_INDEXTYPE i2=1; i2<=j; ++i2) {
            FLENS_DEFAULT_INDEXTYPE j2=j0_y+i2-1;
    ...
    for (FLENS_DEFAULT_INDEXTYPE i1=1; i1<=j; ++i1) {
        FLENS_DEFAULT_INDEXTYPE j1=j0_x+i1-1;
        for (FLENS_DEFAULT_INDEXTYPE i2=1; i2<=j; ++i2) {
...
getSparseGridVector(const Basis &basis, Coefficients<Lexicographical,T,Index3D> &v, FLENS_DEFAULT_INDEXTYPE J, T gamma)
{
    ...
    for (FLENS_DEFAULT_INDEXTYPE i_x=0; i_x<=J; ++i_x) {
        flens::Range<FLENS_DEFAULT_INDEXTYPE> range_x(0,0);
        XType xtype_x;
        FLENS_DEFAULT_INDEXTYPE j_x;
        ...
        for (FLENS_DEFAULT_INDEXTYPE k_x=range_x.firstIndex(); k_x<=range_x.lastIndex(); ++k_x) {
            Index1D index_x(j_x, k_x, xtype_x);

            for (FLENS_DEFAULT_INDEXTYPE i_y=0; i_y<=J-i_x; ++i_y) {
                flens::Range<FLENS_DEFAULT_INDEXTYPE> range_y(0,0);
                XType xtype_y;
                FLENS_DEFAULT_INDEXTYPE j_y;
                ...
                for (FLENS_DEFAULT_INDEXTYPE k_y=range_y.firstIndex(); k_y<=range_y.lastIndex(); ++k_y) {
                    Index1D index_y(j_y, k_y, xtype_y);

                    for (FLENS_DEFAULT_INDEXTYPE i_z=0; i_z<=J-i_x-i_y; ++i_z) {
                        FLENS_DEFAULT_INDEXTYPE sum = i_x+i_y+i_z;
                        FLENS_DEFAULT_INDEXTYPE max = std::max(i_x,std::max(i_y,i_z));
                        if (sum - gamma*max > (1-gamma)*J) continue;
                        flens::Range<FLENS_DEFAULT_INDEXTYPE> range_z(0,0);
                        XType xtype_z;
                        FLENS_DEFAULT_INDEXTYPE j_z;
                        ...
                        for (FLENS_DEFAULT_INDEXTYPE k_z=range_z.firstIndex(); k_z<=range_z.lastIndex(); ++k_z) {
...
{
    FLENS_DEFAULT_INDEXTYPE j0x = basis.first.j0;
    FLENS_DEFAULT_INDEXTYPE j0y = basis.second.j0;
    ...
    FLENS_DEFAULT_INDEXTYPE  jx = index_x.j, jy = index_y.j;
    FLENS_DEFAULT_INDEXTYPE kx = index_x.k, ky = index_y.k;
...
extendMultiTree(const Basis &basis, const Index2D &index2d, IndexSet<Index2D> &Lambda)
{
	...
	if (jx==j0x && index_x.xtype==XWavelet) {
		...
		if(k_scaling_first < k_scaling_last){
			...
			for (FLENS_DEFAULT_INDEXTYPE k_scaling=basis.first.mra.rangeI(j_scaling).firstIndex(); k_scaling<=k_scaling_last; ++k_scaling) {
				PeriodicSupport<typename Basis::T> new_supp_x = basis.first.generator(XBSpline).support(j_scaling,k_scaling);
				if (overlap(supp_x,new_supp_x)>0) {
					Index2D new_index2d(Index1D(j_scaling,k_scaling,XBSpline),index_y);
					//std::cerr << "   extendMultiTree: x-direction check of " << new_index2d << " " << supp_x << " " << new_supp_x << std::endl;
			...
			for (FLENS_DEFAULT_INDEXTYPE k_scaling=k_scaling_first; k_scaling<=basis.first.mra.rangeI(j_scaling).lastIndex(); ++k_scaling) {
				PeriodicSupport<typename Basis::T> new_supp_x = basis.first.generator(XBSpline).support(j_scaling,k_scaling);
				if (overlap(supp_x,new_supp_x)>0) {
					Index2D new_index2d(Index1D(j_scaling,k_scaling,XBSpline),index_y);
					//std::cerr << "   extendMultiTree: x-direction check of " << new_index2d << " " << supp_x << " " << new_supp_x << std::endl;
		...
		if(k_wavelet_first < k_wavelet_last){
			...
			for (FLENS_DEFAULT_INDEXTYPE k_wavelet=basis.first.rangeJ(j_wavelet).firstIndex(); k_wavelet<=k_wavelet_last; ++k_wavelet) {
				PeriodicSupport<typename Basis::T> new_supp_x = basis.first.generator(XWavelet).support(j_wavelet,k_wavelet);
				if (overlap(supp_x,new_supp_x)>0) {
					Index2D new_index2d(Index1D(j_wavelet,k_wavelet,XWavelet),index_y);
					//std::cerr << "   extendMultiTree: x-direction check of " << new_index2d << " " << supp_x << " " << new_supp_x << std::endl;
			...
			for (FLENS_DEFAULT_INDEXTYPE k_wavelet=k_wavelet_first; k_wavelet<=basis.first.rangeJ(j_wavelet).lastIndex(); ++k_wavelet) {
				PeriodicSupport<typename Basis::T> new_supp_x = basis.first.generator(XWavelet).support(j_wavelet,k_wavelet);
				if (overlap(supp_x,new_supp_x)>0) {
					Index2D new_index2d(Index1D(j_wavelet,k_wavelet,XWavelet),index_y);
					//std::cerr << "   extendMultiTree: x-direction check of " << new_index2d << " " << supp_x << " " << new_supp_x << std::endl;
	...
	if (jy==j0y  && index_y.xtype==XWavelet) {
		...
		if(k_wavelet_first < k_wavelet_last){
			...
			for (FLENS_DEFAULT_INDEXTYPE k_wavelet=basis.second.rangeJ(j_wavelet).firstIndex(); k_wavelet<=k_wavelet_last; ++k_wavelet) {
				PeriodicSupport<typename Basis::T> new_supp_y = basis.second.generator(XWavelet).support(j_wavelet,k_wavelet);
				if (overlap(supp_y,new_supp_y)>0) {
					Index2D new_index2d(index_x,Index1D(j_wavelet,k_wavelet,XWavelet));
					//std::cerr << "   extendMultiTree: y-direction check of " << new_index2d << " " << supp_y << " " << new_supp_y << std::endl;
			...
			for (FLENS_DEFAULT_INDEXTYPE k_wavelet=k_wavelet_first; k_wavelet<=basis.second.rangeJ(j_wavelet).lastIndex(); ++k_wavelet) {
				PeriodicSupport<typename Basis::T> new_supp_y = basis.second.generator(XWavelet).support(j_wavelet,k_wavelet);
				if (overlap(supp_y,new_supp_y)>0) {
					Index2D new_index2d(index_x,Index1D(j_wavelet,k_wavelet,XWavelet));
					//std::cerr << "   extendMultiTree: y-direction check of " << new_index2d << " " << supp_y << " " << new_supp_y << std::endl;
...
typename cxxblas::RestrictTo<SFINAE_Wrapper<!IsPeriodic<typename Basis::FirstBasisType>::value
					and !IsPeriodic<typename Basis::SecondBasisType>::value, Basis>::value, void>::Type
extendMultiTree2(const Basis &basis, const Index2D &index2d, const FLENS_DEFAULT_INDEXTYPE offset, IndexSet<Index2D> &Lambda)
{
    ...
    //check x-direction
    if (index_x.j==j0_x) {
        for (FLENS_DEFAULT_INDEXTYPE k=basis.first.mra.rangeI(j0_x).firstIndex(); k<=basis.first.mra.rangeI(j0_x).lastIndex(); ++k) {
        ...
        FLENS_DEFAULT_INDEXTYPE k_first = std::max((FLENS_DEFAULT_INDEXTYPE)basis.first.rangeJ(index_x.j-1).firstIndex(),(FLENS_DEFAULT_INDEXTYPE)index_x.k/2 - offset);
        FLENS_DEFAULT_INDEXTYPE k_last  = std::min((FLENS_DEFAULT_INDEXTYPE)basis.first.rangeJ(index_x.j-1).lastIndex(),(FLENS_DEFAULT_INDEXTYPE)index_x.k/2 + offset);
    ...
    //check y-direction
    if (index_y.j==j0_y) {
        for (FLENS_DEFAULT_INDEXTYPE k=basis.second.mra.rangeI(j0_y).firstIndex(); k<=basis.second.mra.rangeI(j0_y).lastIndex(); ++k) {
        ...
        FLENS_DEFAULT_INDEXTYPE k_first = std::max((FLENS_DEFAULT_INDEXTYPE)basis.second.rangeJ(index_y.j-1).firstIndex(),(FLENS_DEFAULT_INDEXTYPE)index_y.k/2 - offset);
        FLENS_DEFAULT_INDEXTYPE k_last  = std::min((FLENS_DEFAULT_INDEXTYPE)basis.second.rangeJ(index_y.j-1).lastIndex(),(FLENS_DEFAULT_INDEXTYPE)index_y.k/2 + offset);
        for (FLENS_DEFAULT_INDEXTYPE k=k_first; k<=k_last; ++k) {
...
extendMultiTree2(const Basis &basis, const Index2D &index2d, const FLENS_DEFAULT_INDEXTYPE offset, IndexSet<Index2D> &Lambda)
{
	...
	if (index_x.j==j0_x) {
		...
		FLENS_DEFAULT_INDEXTYPE k_first = (FLENS_DEFAULT_INDEXTYPE)index_x.k/2 - offset;
		FLENS_DEFAULT_INDEXTYPE k_last = (FLENS_DEFAULT_INDEXTYPE)index_x.k/2 + offset;
		bool is_wrap = false;
		while(k_first < 1){
			k_first = (FLENS_DEFAULT_INDEXTYPE)basis.first.rangeJ(index_x.j-1).lastIndex() + k_first;
			is_wrap = true;
		}
		while(k_last > (FLENS_DEFAULT_INDEXTYPE)basis.first.rangeJ(index_x.j-1).lastIndex()){
			k_last = k_last - (FLENS_DEFAULT_INDEXTYPE)basis.first.rangeJ(index_x.j-1).lastIndex();
		   is_wrap = true;
		}
		if(k_first == k_last || k_first == k_last+1 || (k_first < k_last && is_wrap)){
			k_first = (FLENS_DEFAULT_INDEXTYPE)basis.first.rangeJ(index_x.j-1).firstIndex();
			k_last = (FLENS_DEFAULT_INDEXTYPE)basis.first.rangeJ(index_x.j-1).lastIndex();
		}
		...
		if(k_first < k_last){
			...
			for (FLENS_DEFAULT_INDEXTYPE k=(FLENS_DEFAULT_INDEXTYPE)basis.first.rangeJ(index_x.j-1).firstIndex(); k<=k_last; ++k) {
				PeriodicSupport<typename Basis::T> new_supp_x = basis.first.generator(XWavelet).support(index_x.j-1,k);
				if (overlap(supp_x,new_supp_x)>0) {
					Index2D new_index2d(Index1D(index_x.j-1,k,XWavelet),index_y);
					//std::cerr << "   extendMultiTree: x-direction check of " << new_index2d << " " << supp_x << " " << new_supp_x << std::endl;
			...
			for (FLENS_DEFAULT_INDEXTYPE k=k_first; k<=(FLENS_DEFAULT_INDEXTYPE)basis.first.rangeJ(index_x.j-1).lastIndex(); ++k) {
				PeriodicSupport<typename Basis::T> new_supp_x = basis.first.generator(XWavelet).support(index_x.j-1,k);
				if (overlap(supp_x,new_supp_x)>0) {
					Index2D new_index2d(Index1D(index_x.j-1,k,XWavelet),index_y);
					//std::cerr << "   extendMultiTree: x-direction check of " << new_index2d << " " << supp_x << " " << new_supp_x << std::endl;
	...
	if (index_y.j==j0_y) {
		...
		FLENS_DEFAULT_INDEXTYPE k_first = (FLENS_DEFAULT_INDEXTYPE)index_y.k/2 - offset;
		FLENS_DEFAULT_INDEXTYPE k_last = (FLENS_DEFAULT_INDEXTYPE)index_y.k/2 + offset;
		bool is_wrap = false;
		while(k_first < 1){
			k_first = (FLENS_DEFAULT_INDEXTYPE)basis.second.rangeJ(index_y.j-1).lastIndex() + k_first;
			is_wrap = true;
		}
		while(k_last > (FLENS_DEFAULT_INDEXTYPE)basis.second.rangeJ(index_y.j-1).lastIndex()){
			k_last = k_last - (FLENS_DEFAULT_INDEXTYPE)basis.second.rangeJ(index_y.j-1).lastIndex();
		   is_wrap = true;
		}
		if(k_first == k_last || k_first == k_last+1 || (k_first < k_last && is_wrap)){
			k_first = (FLENS_DEFAULT_INDEXTYPE)basis.second.rangeJ(index_y.j-1).firstIndex();
			k_last = (FLENS_DEFAULT_INDEXTYPE)basis.second.rangeJ(index_y.j-1).lastIndex();
		}
		...
		if(k_first < k_last){
			...
			for (FLENS_DEFAULT_INDEXTYPE k=(FLENS_DEFAULT_INDEXTYPE)basis.second.rangeJ(index_y.j-1).firstIndex(); k<=k_last; ++k) {
				PeriodicSupport<typename Basis::T> new_supp_y = basis.second.generator(XWavelet).support(index_y.j-1,k);
				if (overlap(supp_y,new_supp_y)>0) {
					Index2D new_index2d(index_x,Index1D(index_y.j-1,k,XWavelet));
					//std::cerr << "   extendMultiTree: y-direction check of " << new_index2d << " " << supp_y << " " << new_supp_y << std::endl;
			...
			for (FLENS_DEFAULT_INDEXTYPE k=k_first; k<=(FLENS_DEFAULT_INDEXTYPE)basis.second.rangeJ(index_y.j-1).lastIndex(); ++k) {
				PeriodicSupport<typename Basis::T> new_supp_y = basis.second.generator(XWavelet).support(index_y.j-1,k);
				if (overlap(supp_y,new_supp_y)>0) {
					Index2D new_index2d(index_x,Index1D(index_y.j-1,k,XWavelet));
					//std::cerr << "   extendMultiTree: y-direction check of " << new_index2d << " " << supp_y << " " << new_supp_y << std::endl;
...
{
    FLENS_DEFAULT_INDEXTYPE k_first_x, k_last_x, k_first_y, k_last_y;
    FLENS_DEFAULT_INDEXTYPE j_x, j_y;
    for(IndexSet<Index2D>::const_iterator it = indexset_origin.begin(); it != indexset_origin.end(); ++it){
		...
		if(k_first_x < k_last_x){
			for(FLENS_DEFAULT_INDEXTYPE k_new_x = k_first_x; k_new_x <= k_last_x; ++k_new_x){
				Index1D ind_x(j_x,k_new_x,(*it).index1.xtype);

				// Inner loop: all indizes in y-direction
				if(k_first_y < k_last_y){
					for(FLENS_DEFAULT_INDEXTYPE k_new_y = k_first_y; k_new_y <= k_last_y; ++k_new_y){
					...
					if((*it).index2.xtype==XBSpline){
						for(FLENS_DEFAULT_INDEXTYPE k_new_y = k_first_y; k_new_y <= basis_target.second.mra.rangeI(j_y).lastIndex(); ++k_new_y){
							Index1D ind_y(j_y, k_new_y, (*it).index2.xtype);
							completeMultiTree(basis_target, Index2D(ind_x, ind_y), coeffs_target, 0, true);
						}
						for(FLENS_DEFAULT_INDEXTYPE k_new_y = basis_target.second.mra.rangeI(j_y).firstIndex(); k_new_y <= k_last_y; ++k_new_y){
							Index1D ind_y(j_y, k_new_y, (*it).index2.xtype);
							completeMultiTree(basis_target, Index2D(ind_x, ind_y), coeffs_target, 0, true);
						}
					}
					else{
						for(FLENS_DEFAULT_INDEXTYPE k_new_y = k_first_y; k_new_y <= basis_target.second.rangeJ(j_y).lastIndex(); ++k_new_y){
							Index1D ind_y(j_y, k_new_y, (*it).index2.xtype);
							completeMultiTree(basis_target, Index2D(ind_x, ind_y), coeffs_target, 0, true);
						}
						for(FLENS_DEFAULT_INDEXTYPE k_new_y = basis_target.second.rangeJ(j_y).firstIndex(); k_new_y <= k_last_y; ++k_new_y){
							Index1D ind_y(j_y, k_new_y, (*it).index2.xtype);
							completeMultiTree(basis_target, Index2D(ind_x, ind_y), coeffs_target, 0, true);
						}
					}
			...
			if((*it).index1.xtype==XBSpline){
				for(FLENS_DEFAULT_INDEXTYPE k_new_x = k_first_x; k_new_x <= basis_target.first.mra.rangeI(j_x).lastIndex(); ++k_new_x){
					Index1D ind_x(j_x,k_new_x,(*it).index1.xtype);
				...
				for(FLENS_DEFAULT_INDEXTYPE k_new_x = basis_target.first.mra.rangeI(j_x).firstIndex(); k_new_x <= k_last_x; ++k_new_x){
					Index1D ind_x(j_x,k_new_x,(*it).index1.xtype);
				...
				for(FLENS_DEFAULT_INDEXTYPE k_new_x = k_first_x; k_new_x <= basis_target.first.rangeJ(j_x).lastIndex(); ++k_new_x){
					Index1D ind_x(j_x,k_new_x,(*it).index1.xtype);
				...
				for(FLENS_DEFAULT_INDEXTYPE k_new_x = basis_target.first.rangeJ(j_x).firstIndex(); k_new_x <= k_last_x; ++k_new_x){
					Index1D ind_x(j_x,k_new_x,(*it).index1.xtype);
				   ...
				   IndexSet<Index2D>& indexset_origin, Coefficients<Lexicographical,T,Index2D>& coeffs_target)
{
    ...
    for(IndexSet<Index2D>::const_iterator it = indexset_origin.begin(); it != indexset_origin.end(); ++it){
		...
		if((*it).index1.xtype==XBSpline){
			...
			FLENS_DEFAULT_INDEXTYPE k_s_first, k_s_last;
			FLENS_DEFAULT_INDEXTYPE j_s;
			...
			FLENS_DEFAULT_INDEXTYPE k_w_first, k_w_last;
			if(k_s_first < k_s_last){
				for(FLENS_DEFAULT_INDEXTYPE k = k_s_first; k <= k_s_last; ++k){
				...
				for(FLENS_DEFAULT_INDEXTYPE k = k_s_first; k <= basis_origin.first.rangeJ(j_s).lastIndex(); ++k){
					basis_origin.first.getHigherWaveletNeighborsForWavelet(j_s,k,basis_target.first,j_x,k_w_first, k_w_last);
					if(k_w_first < k_first_x){
						k_first_x = k_w_first;
					}
				...
				for(FLENS_DEFAULT_INDEXTYPE k = basis_origin.first.rangeJ(j_s).firstIndex(); k <= k_s_last; ++k){
					basis_origin.first.getHigherWaveletNeighborsForWavelet(j_s,k,basis_target.first,j_x,k_w_first, k_w_last);
					if(k_w_first < k_first_x){
						k_first_x = k_w_first;
					}
		...
		if((*it).index2.xtype==XBSpline){
			...
			if(k_s_first < k_s_last){
				...
				for(FLENS_DEFAULT_INDEXTYPE k = k_s_first; k <= basis_origin.second.rangeJ(j_s).lastIndex(); ++k){
					basis_origin.second.getHigherWaveletNeighborsForWavelet(j_s,k,basis_target.second,j_y,k_w_first, k_w_last);
					if(k_w_first < k_first_y){
						k_first_y = k_w_first;
					}
				...
				for(FLENS_DEFAULT_INDEXTYPE k = basis_origin.second.rangeJ(j_s).firstIndex(); k <= k_s_last; ++k){
					basis_origin.second.getHigherWaveletNeighborsForWavelet(j_s,k,basis_target.second,j_y,k_w_first, k_w_last);
					if(k_w_first < k_first_y){
						k_first_y = k_w_first;
					}
				   ...
				   IndexSet<Index2D>& indexset_origin, Coefficients<Lexicographical,T,Index2D>& coeffs_target)
{
    ...
    FLENS_DEFAULT_INDEXTYPE k_first_x, k_last_x;
    FLENS_DEFAULT_INDEXTYPE j_x;
    for(IndexSet<Index2D>::const_iterator it = indexset_origin.begin(); it != indexset_origin.end(); ++it){
