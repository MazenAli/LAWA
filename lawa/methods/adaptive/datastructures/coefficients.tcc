...
template <typename T, typename Index>
T
dotProduct(const Coefficients<Lexicographical, T, Index>& v1,
           const Coefficients<Lexicographical, T, Index>& v2)
{
    T sum = 0.;
    if (v1.size()<=v2.size()) {
        for (const auto& it : v1) {
            auto get = v2.find(it.first);
            if (get!=v2.end()) {
                sum += it.second*get->second;
            }
        }
    } else {
        for (const auto& it : v2) {
            auto get = v1.find(it.first);
            if (get!=v1.end()) {
                sum += it.second*get->second;
            }
        }
