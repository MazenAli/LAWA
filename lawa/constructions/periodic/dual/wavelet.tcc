...
template <typename T>
Wavelet<T,Dual,Periodic,CDF>::Wavelet(FLENS_DEFAULT_INDEXTYPE _d, FLENS_DEFAULT_INDEXTYPE _d_)
    : d(_d), d_(_d_), mu(d&1),
      psiR_(_d, _d_)
{
    assert(d<=d_);
...
template <typename T>
T
Wavelet<T,Dual,Periodic,CDF>::operator()(T x, FLENS_DEFAULT_INDEXTYPE j, FLENS_DEFAULT_INDEXTYPE k, unsigned short deriv) const
{
    assert(deriv==0);
    ...
    // = 'wrapping' around [0,1]
    T val = 0;
    for(FLENS_DEFAULT_INDEXTYPE l = ifloor(psiR_.support(j,k).l1); l < iceil<FLENS_DEFAULT_INDEXTYPE>(psiR_.support(j,k).l2); ++l){
        val += psiR_(l+x, j, k);
    }
    return val;
}
...
template <typename T>
PeriodicSupport<T>
Wavelet<T,Dual,Periodic,CDF>::support(FLENS_DEFAULT_INDEXTYPE j, FLENS_DEFAULT_INDEXTYPE k) const
{
...
Wavelet<T,Dual,Periodic,CDF>::mask(FLENS_DEFAULT_INDEXTYPE d, FLENS_DEFAULT_INDEXTYPE d_)
{
    assert(d<=d_);
    assert(((d+d_)&1)==0);
    ...
    for (FLENS_DEFAULT_INDEXTYPE k=b_.firstIndex(); k<=b_.lastIndex(); ++k) {
