...
evaluate(const MRA<typename X::ElementType,Primal,Periodic,CDF> &mra, FLENS_DEFAULT_INDEXTYPE j,
         const flens::DenseVector<X> &coeffs, typename X::ElementType x, FLENS_DEFAULT_INDEXTYPE deriv)
{
    ...
    FLENS_DEFAULT_INDEXTYPE offsetI = mra.rangeI(mra.j0).firstIndex()-coeffs.firstIndex();
    T ret = 0.0;
         ...
         FLENS_DEFAULT_INDEXTYPE deriv)
{
    ...
    basis.setLevel(j0);
    T ret = 0;
    FLENS_DEFAULT_INDEXTYPE offsetJ = basis.rangeJ(j0).firstIndex()-coeffs.firstIndex();
    flens::Range<FLENS_DEFAULT_INDEXTYPE> range(coeffs.firstIndex(), coeffs.firstIndex() + basis.cardJ(j0) - 1);
