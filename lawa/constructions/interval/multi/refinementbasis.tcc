...
template <typename T>
Basis<T,Orthogonal,Interval,MultiRefinement>::Basis(FLENS_DEFAULT_INDEXTYPE _d, FLENS_DEFAULT_INDEXTYPE j)
...
template <typename T>
void
Basis<T,Orthogonal,Interval,MultiRefinement>::setLevel(FLENS_DEFAULT_INDEXTYPE j) const
{
    assert(j>=j0);
    _j = j;
}
                              ...
                              FLENS_DEFAULT_INDEXTYPE &j_bspline2, FLENS_DEFAULT_INDEXTYPE &k_bspline2_first, FLENS_DEFAULT_INDEXTYPE &k_bspline2_last) const
{
    ...
    if (b<1.L) {
        k_bspline2_first = std::max(k_bspline1 - d + 1, (FLENS_DEFAULT_INDEXTYPE) 0);
        k_bspline2_last  = std::min(k_bspline1 + d - 1, (FLENS_DEFAULT_INDEXTYPE)secondrefinementbasis.mra.rangeI(j_bspline2).lastIndex());
        return;
    }
    k_bspline2_first = std::max((FLENS_DEFAULT_INDEXTYPE) 0,k_bspline1 - d);
                                ...
                                FLENS_DEFAULT_INDEXTYPE &j_wavelet, FLENS_DEFAULT_INDEXTYPE &k_wavelet_first, FLENS_DEFAULT_INDEXTYPE &k_wavelet_last) const
{
    ...
    if (0<a && b<1.L) {
        ...
        k_wavelet_first = std::max((FLENS_DEFAULT_INDEXTYPE)secondbasis.rangeJL(j_wavelet).firstIndex(),k_wavelet_first);
        k_wavelet_last  = std::min((FLENS_DEFAULT_INDEXTYPE)secondbasis.rangeJR(j_wavelet).lastIndex(), k_wavelet_last);
        return;
    }
...
LaplaceOperator1D(FLENS_DEFAULT_INDEXTYPE _d, const Basis<T,Orthogonal,Interval,MultiRefinement> &_refinementbasis)
 : d(_d), refinementbasis(_refinementbasis)
{
    switch (d) {
        case 1:
            ...
            break;
        case 2:
            inner_values1.engine().resize((FLENS_DEFAULT_INDEXTYPE)2,0);
            inner_values1 = 2., -1.;
            break;
        case 3:
            outer_values.engine().resize((FLENS_DEFAULT_INDEXTYPE)3,0);
            outer_values = 4.L/3.L, -1.L/6.L, -1.L/6.L;
            inner_values1.engine().resize((FLENS_DEFAULT_INDEXTYPE)3,0);
            inner_values1 = 1.L, -1.L/3.L, -1.L/6.L;
            break;
        case 4:
            outer_values.engine().resize((FLENS_DEFAULT_INDEXTYPE)4,0);
            outer_values = 6.L/5.L, 0.L, -3.L/10.L, 0.L;
            inner_values1.engine().resize((FLENS_DEFAULT_INDEXTYPE)4,0);
            inner_values1 = 6.L/5.L, 0.L, -3.L/8.L, -3.L/40.L;
            inner_values2.engine().resize((FLENS_DEFAULT_INDEXTYPE)4,0);
...
IdentityOperator1D(FLENS_DEFAULT_INDEXTYPE _d, const Basis<T,Orthogonal,Interval,MultiRefinement> &_refinementbasis)
 : d(_d), refinementbasis(_refinementbasis)
{
    switch (d) {
        case 1:
